...
% Compute gradient to get the heat flux
[Tx, Ty] = gradient(t_now);

% Plot heat flux vectors
figure(2);
quiver(x, y, -Tx, -Ty);
title('Heat flux vectors (Dirichlet BC)');
xlabel('x');
ylabel('y');
axis tight equal;
...
figure(3);
surf(x,y,t_now);
title('Temperature distribution (Dirichlet BC)');
xlabel('x');
ylabel('y');
zlabel('Temperature');
...
figure(6);
quiver(x, y, -Tx, -Ty);
title('Heat flux vectors (Mixed BC)');
xlabel('x');
ylabel('y');
axis tight equal;
...
figure(7);
surf(x,y,t_now);
title('Temperature distribution (Mixed BC)');
xlabel('x');
ylabel('y');
zlabel('Temperature');
